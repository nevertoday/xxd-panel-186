...
# XXD Panel 186｜شبكة ذكريات هندسية من تسع خلايا
...
شبكة ذكريات هندسية من تسع خلايا
...
- `design-only`: تعرض اللوحة الكاملة ترجمة Panel 186 التصميمية فقط، وتبقى الصورة مرجعاً غير ظاهر.
...
بعد التثبيت أعد تشغيل جلسة Agent ثم استدعِ `$xxd-panel-186`. ويمكن إضافة `--global --agent codex --yes` للتثبيت على مستوى المستخدم.

```text
/xxd-panel-186 photo.jpg --mode top-bottom --size 3:4 --text prompt --locale ar-SA
...
راجع [SKILL.md](SKILL.md) لعقد التشغيل الكامل، ومهايئ التشغيل [بالإنجليزية](references/xxd-panel-186-prompt.en.md) أو [بالصينية](references/xxd-panel-186-prompt.zh-CN.md).
